...
def transition(Sun,cusp):#should return true when the sun is in 1,12,6 or 7 (cusp is a list of all houses)
	...
	def isin(point, limit_a,limit_b):#limit_a should be authorised to be > limit_b and still check the small angle
		...
		if not attrib and not vernal_point_in_range:
			return False
	attrib_2=False
	if isin(Sun,cusp[0][3],cusp[0][10]):
		attrib_2=True
		print "dawn (sun in 12 or 1)"
		return True
	if isin(Sun,cusp[0][5],cusp[0][7]):
		print"dusk (sun in 7 or 6)"
		attrib_2=True
		return True	
	if not attrib_2: 
		print"day or night (no transition)"
		return False
	...
	for word in range(len(match)):
		...
		if word<=len (match)-1:
			if "Pss." in matchref[word] or True:#condition canceled after data storing implementation
				...
				if True:# preceding condition canceled
					count=count+1
					row_to_add[0]=count
...
def search_any_heb_combi(charlist):	#works with any number of characters 
	...
	#printing pss 127
	#print "printing pss 127"
	for i in range(len(verset_list)):
		#print unicode(verset_list[i][0])
		#if "Pss. 127" in unicode(verset_list[i][0]):
			#print verset_list [i][0],verset_list [i][2]
		pass
	...
	for verset in verset_list:#cycling thru lines in bible
		#print "line"
		k+=1
		if "Pss. 127:2" in verset[0]:
			#print verset[0],k
			#print verset[2]
			#for word in verset[2].split():
				#print unicode(word)
			pass
		line=verset[2].split()#2 is without vowels
...
#print "Sun zodiaque sign",zodiaque
...
#sign_asc=int(longitude_asc//30)+1
#print "ascendant",sign_asc
...
#print "transition", transition(longitude_sun, cusp)
#transition_=False
#enjournee=True
#if transition_: 
